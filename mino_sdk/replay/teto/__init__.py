...
The native Python simulator (:mod:`mino_sdk.sim`) is frame-accurate for most
...
teto frame-by-frame from a decoded :class:`~mino_sdk.replay.model.Replay` and
returns the reconstructed stats, final board, and per-lock events as JSON. It is
the ``engine="teto"`` path of :func:`mino_sdk.replay.simulate.simulate`.
