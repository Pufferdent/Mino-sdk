...
:func:`simulate` runs a decoded replay through the :mod:`tetris_sdk.sim` stack —
...
:mod:`tetris_sdk.replay.teto`) for an exact, frame-perfect reconstruction — the
...
from tetris_sdk.engine import Move, SpinType, translate
from tetris_sdk.events import B2BRule, Event, classify_lock, is_difficult
from tetris_sdk.pieces import SRS, SRSPlus, PieceType, RotationSystem
from tetris_sdk.replay.model import Platform, Replay, ReplayInput
from tetris_sdk.types import Cell
from tetris_sdk.sim.engine import (
    GameState,
    hard_drop,
    hold_piece,
    move,
...
from tetris_sdk.sim.gravity import GravityProfile, gravity_for
from tetris_sdk.sim.handling import FLOOR, WALL, HandlingState, Motion
from tetris_sdk.sim.queue import Hold, Queue
from tetris_sdk.sim.rng import JstrisRng, Rng, TetrioRng
...
def _simulate_teto(replay: Replay) -> ReplaySim:
    ...
    from tetris_sdk.replay.teto import run_teto
...
def simulate(
    ...
    * ``"native"`` (default) — the pure-Python :mod:`tetris_sdk.sim` stack;
    ...
    * ``"teto"`` — the real ``@haelp/teto`` TETR.IO engine (TETR.IO replays
      ...
      runner's dependencies (see :mod:`tetris_sdk.replay.teto`).
    """
    if engine == "teto":
