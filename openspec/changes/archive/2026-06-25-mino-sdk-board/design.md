...
The Mino SDK is a greenfield Python project. The first building blocks are the Board model and the ability to import board states from fumen strings — a community-standard format for sharing board configurations (used by tools like fumen.zui.jp). Modern (guideline) stackers use a 40-row × 10-column playfield. Implementation is pure Python with no external dependencies.
...
Row 0 is the bottom (visible playfield), row 39 is the top (hidden rows above the skyline). This is conventional for stacker implementations and maps naturally to visual rendering.
