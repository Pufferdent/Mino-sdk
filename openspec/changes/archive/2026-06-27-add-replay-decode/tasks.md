...
- [x] 1.1 Create `mino_sdk/replay/model.py` with `Platform`, `ReplayInput` enums
...
- [x] 2.1 Create `mino_sdk/replay/tetrio.py` with the `data.key → ReplayInput` map
...
- [x] 3.1 Create `mino_sdk/replay/jstris.py`; LZString-decompress to JSON `{c, d}`; populate `ReplayMeta` from `c`
...
- [x] 4.1 Implement `decode_replay(data)` in `mino_sdk/replay/__init__.py` with path/str/bytes input and platform auto-detection; raise `ValueError` on unknown
- [x] 4.2 Re-export `decode_tetrio`, `decode_jstris`, and the model types from the package
- [x] 4.3 Export `Replay`, `ReplayMeta`, `InputEvent`, `ReplayInput`, `Platform`, `Handling`, `decode_replay` from `mino_sdk/__init__.py`
