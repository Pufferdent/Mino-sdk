...
- [x] 1.1 Add piece type mapping tables (`FUMEN_PIECE_TO_TYPE`, `TYPE_TO_FUMEN_PIECE`) to `tetris_sdk/fumen/decoder.py` and export from package
- [x] 1.2 Create `tetris_sdk/coords.py` with coordinate conversion utilities (`fumen_row_to_board_row`, `board_row_to_fumen_row`, `fumen_position_to_row_col`)
- [x] 1.3 Export new symbols from `tetris_sdk/__init__.py`

## 2. Fumen Pipeline — MultiFumenPage

- [x] 2.1 Create `tetris_sdk/fumen/multi_fumen.py` with `Page` dataclass (board, comment) and `MultiFumenPage` class
...
- [x] 3.1 Add `board_to_string()` to `tetris_sdk/board.py`: convert bottom 4 rows to 40-char string (top-to-bottom, N/X/piece glyphs)
- [x] 3.2 Add `board_from_string()` to `tetris_sdk/board.py`: parse 40-char string to Board cells (row 0 = bottom)
- [x] 3.3 Add round-trip tests
- [x] 3.4 Export from package

## 4. Fumen Pipeline — Piece Shape Computation

- [x] 4.1 Add `get_piece_cells(type, rotation, x, y, coord_system='sdk')` to `tetris_sdk/pieces.py`
...
- [x] 5.1 Create `tetris_sdk/fumen/encoder.py` with `encode_fumen(pages)` function
...
- [x] 7.1 Create `tetris_sdk/solver/queue_validator.py` with `is_placement_order_valid()` and `enumerate_placement_orders()`
...
- [x] 8.1 Create `tetris_sdk/solver/core.py` with `solve_pc()` function
...
- [x] 9.1 Create `tetris_sdk/solver/expressions.py` with tokenizer, recursive-descent parser, and AST node classes
...
- [x] 10.1 Create `tetris_sdk/solver/saves.py` with save percentage computation and solve filtering
...
- [x] 11.1 Create `tetris_sdk/solver/__init__.py` exporting all public symbols
- [x] 11.2 Import and re-export solver symbols from `tetris_sdk/__init__.py`
- [x] 11.3 Add solver module to package setup/config if needed

## 12. Integration & Polish

- [x] 12.1 Review all exports in `tetris_sdk/__init__.py` for consistency
