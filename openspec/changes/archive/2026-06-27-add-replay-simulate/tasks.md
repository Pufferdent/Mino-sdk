...
- [x] 1.1 Create `tetris_sdk/sim/rng.py` with an `Rng` protocol, `TetrioRng`, `JstrisRng` (seeded, deterministic)
...
- [x] 1.3 Create `tetris_sdk/sim/queue.py` with `Queue` (7-bag from an `Rng`, preview) and `Hold` (once-per-piece, resets on lock); includes the TETR.IO `no_szo` first-bag rule

## 2. Handling (sim-handling)

- [x] 2.1 Create `tetris_sdk/sim/handling.py` — a state machine consuming `InputEvent`s and emitting per-frame horizontal/soft-drop motion from `Handling(das, arr, sdf, dcd)`
...
- [x] 3.1 Create `tetris_sdk/sim/gravity.py` with `GravityProfile(g, lock_delay, lock_resets, is_20g[, ramp])`
...
- [x] 4.1 Create `tetris_sdk/sim/engine.py` with `GameState(board, queue, hold, active, frame)` spawning the first piece
...
- [x] 5.1 Create `tetris_sdk/replay/simulate.py` with `simulate(replay, *, gravity=None, engine="native") -> ReplaySim`
...
- [x] 5.4 Implement the `engine="teto"` path: route TETR.IO replays to the real `@haelp/teto` engine (`tetris_sdk/replay/teto/`) for exact frame-perfect reconstruction

## 6. Public API & tests

- [x] 6.1 Export `simulate`, `ReplaySim`, `GravityProfile`, gravity registry/`gravity_for`, `TetrioRng`, `JstrisRng`, `GameState` from `tetris_sdk/__init__.py`
...
- **Engine rotation fix**: the base `_rotate_cw` in `tetris_sdk/pieces.py` rotated counter-clockwise (only the I piece matched, by spawn-row coincidence), mirroring J/L/S/T/Z placements. Fixed to a true clockwise transform with the I piece on the guideline spawn row; the affected `pieces`/`events` tests were updated.
- **`engine="teto"`** requires Node.js on `PATH` and `npm install` in `tetris_sdk/replay/teto/`. The native engine is frame-accurate for most play but cannot match TETR.IO's exact sub-frame loop on demanding replays (PC loops, dense soft-drop tucks); `teto` is the documented exact fallback.
