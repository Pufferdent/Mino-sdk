...
- [x] 1.1 Create `tetris_sdk/` package directory with `__init__.py`
- [x] 1.2 Create `tetris_sdk/types.py` with `Cell` IntEnum (EMPTY=0, T=1, I=2, L=3, J=4, S=5, Z=6, O=7, GARBAGE=8, SOLID=9)
...
- [x] 3.1 Create `tetris_sdk/fumen/` package with `__init__.py`
- [x] 3.2 Implement `tetris_sdk/fumen/decoder.py` with low-level helpers: version extraction, base64 decode, zlib decompress, RLE decoding of field data
- [x] 3.3 Implement `tetris_sdk/fumen/parser.py` with `parse_fumen(fumen_str: str) -> list[Board]` public API
