...
Reproduce each platform's exact piece sequence from a replay seed. Provides the
seeded `Rng` implementations, the 7-bag `Queue` (with preview), and the `Hold`
slot that the frame engine draws from during reconstruction.
