...
Turn a replay's held-key intervals into realized per-frame motion. A handling
state machine consumes `InputEvent` press/release edges and applies DAS/ARR/SDF
behavior so the simulator moves pieces exactly as the player's settings dictate.
